...
К примеру, одним из наиболее популярных решений для парсинга html является библиотека [Jsoup](http://jsoup.org), содержащая довольно
...
вам [доступен](https://jsoup.org/cookbook/extracting-data/selector-syntax) полный синтаксис селекторов этой библиотеки.
...
кроме списка статей со страницы нам необходимо получить ссылки на некоторые страницы компании в социальных сетях, а класс статьи так же должен содержать поле с именем файла превью-картинки из статьи.
...
тега `div` имеющего класс `news__item`, то нам остается лишь найти тег `img` внутри тега `a`, получить его аттрибут `src` и 
извлечь имя файла из ссылки с помощью простого регулярного выражения. Обновленная версия класса, представляющего статью, имеет следующий вид:
~~~ java
...
Использование jspoon для парсинга значительно сокращает объем кода, требуемый для парсинга необходимых вам данных, а 
богатый возможностями синтаксис селекторов и возможность использования регулярных выражений позволят извлекать данные из html-кода практически любой сложности.
